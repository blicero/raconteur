...
// Time-stamp: <2021-09-12 00:03:37 krylon>
...
EXEC_QUERY:
	if res, err = stmt.Exec(f.ProgramID, f.Path); err != nil {
		if worthARetry(err) {
			waitForRetry()
			goto EXEC_QUERY
		} else {
...
EXEC_QUERY:
	...
	if rows.Next() {
		...
		if err = rows.Scan(&f.ProgramID, &f.Path, &f.Title, &f.Position, &stamp); err != nil {
			db.log.Printf("[ERROR] Cannot scan row: %s\n", err.Error())
			return nil, err
		}
...
EXEC_QUERY:
	...
	if rows.Next() {
		...
		if err = rows.Scan(&f.ID, &f.ProgramID, &f.Title, &f.Position, &stamp); err != nil {
			db.log.Printf("[ERROR] Cannot scan row: %s\n", err.Error())
			return nil, err
		}
